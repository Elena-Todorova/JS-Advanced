class Garden {
    ...
    generateReport(){
        ...
        let plant = this.plants.map(x => x.plantName).join(', ');
        result.push(`Plants in the garden: ${plant}`);

        let storageStr = this.storage.map(p => `${p.plantName} (${p.quantity})`).join(', ');

        if(this.storage.length === 0){
            result.push("Plants in storage: The storage is empty.");
        } else{
            result.push(`Plants in storage: ${storageStr}`);
        };
